...
- Add hardware acceleration only as a real runtime provider. XReader's active target is a Qualcomm QNN development build; no acceleration toggle should appear until provider selection, no-CPU-fallback behavior, prepared-model compatibility, and generation speed are proven on device.
