...
The current full-book embedded path requires measured acceleration. It benchmarks eligible Sherpa-ONNX providers in order: strict QNN GPU, prepared strict QNN HTP/NPU, then strict NNAPI. CPU-backed `xnnpack`/`cpu` are excluded from full-book generation, and providers that are unavailable, missing required model artifacts, fail initialization, assign graph nodes to CPU, or run slower than the full-book realtime threshold fail closed instead of silently generating unusably slow audio. Prepared QNN HTP/NPU models require both `model.qnn.onnx` and a matching `xreader-qnn-model-manifest.json` with `strict_qnn_compatible: true`; a bare quantized artifact is treated as incomplete. Preview generation can still fall back to `xnnpack`/`cpu` for short voice checks. Long generation remains isolated in the `:audiobook_generation` process so native provider failures do not kill the main reader UI.
