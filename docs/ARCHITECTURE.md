...
- `BookAudioEntity`
- `ImportedAudiobookEntity` with ordered tracks, chapters, bookmarks, and collection membership
- `PronunciationRuleEntity` and per-book `NarrationOverrideEntity`
- `RestoreOperationEntity`

Search uses a normal table plus an FTS table. Book deletion removes search rows and stored files.

The Room schema is version 16. Migration 14 to 15 removes only the redundant B-tree index on `search_index.normalizedBody`; the FTS table remains authoritative for full-text queries. Migration 15 to 16 adds imported-audiobook, track, chapter, bookmark, collection-membership, pronunciation-rule, narration-override, and restore-operation tables without changing `BookEntity` or inventing an audio `BookFormat`. The database registers the complete 1 to 16 chain, exports every schema, and compiles instrumentation coverage for every historical starting version plus representative retained rows. Runtime migration execution remains a connected-device gate.
...
Imported audiobooks use a separate persistence and storage path. `AudiobookImportService` validates selected media with `MediaMetadataRetriever`, calculates ordered track and publication hashes, extracts bounded embedded metadata/artwork and supported chapter tags, and commits metadata only after the media set is copied into app-private storage. A replacement keeps the previous directory until the Room transaction succeeds, so a failed repair can roll storage back. `ImportedAudiobookPlaybackController` uses Android `MediaPlayer`, audio focus, a dedicated MediaSession, persisted track/offset, and a media-playback foreground service. Generated and imported audio remain separate storage/generation lifecycles even when presented in the same Audiobooks destination.

Incoming Open with/Share intents are classified before import. Supported audio goes to the audiobook importer, supported ebook/document files remain on the library path, and ACSM license instructions show an external-app handoff instead of entering conversion or Readium.
...
Embedded neural audiobook generation is handled separately by `NeuralTtsRepository`. Settings exposes a compact local neural voice downloader backed by the Sherpa-ONNX Android JNI runtime and the Kokoro v1.0 model. Downloads are stored in app-private model storage, progress is persisted in Room, archive size and SHA-256 are verified before extraction, and extraction rejects unsafe archive paths. `NarrationPreparationReport` preserves source positions, warnings, chapter/segment output, estimated duration, applied exact-phrase pronunciations, and exclusions with reasons. Only adjacent extraction duplicates are collapsed; repeated headers/footers and front matter require structural evidence, while non-adjacent refrains and repeated dialogue remain. The pre-generation review persists per-book restore/exclude decisions, and unsupported languages block the English Kokoro profile instead of claiming multilingual quality. Generation then synthesizes the selected scope to bounded WAV segments with heartbeat progress, provider evidence, chapter/pause sidecars, resume, partial playback, delete, and ZIP export. The global Audiobooks screen continues to materialize verified generated-file metadata on IO rather than Compose.

The current embedded neural path requires measured QNN acceleration. It allows one strict QNN HTP/NPU provider only; `qnn-gpu`, `nnapi`, `webgpu`, `xnnpack`, and `cpu` are excluded as alternate preview and full-book generation backends and GPU/OpenCL runtime libraries are not packaged. Runtime initialization attempts that selected QNN provider, and providers that are unavailable, missing required model artifacts, fail initialization, assign graph nodes to CPU, or run slower than the full-book audio-time threshold fail closed. Strict QNN providers require both `model.qnn.onnx` and a matching `xreader-qnn-model-manifest.json`; the validator checks model hash/size, source identity/revision, ordered fixed token buckets, ONNX/ONNX Runtime/QAIRT versions, blocker analysis, and provenance/license fields instead of trusting one compatibility boolean. Long generation remains isolated in the `:audiobook_generation` process.
...
Settings exposes a versioned `.xreader-backup` ZIP through Android's Storage Access Framework. Its manifest declares app/schema version, export/settings capture times, section paths, record counts, byte sizes, and SHA-256 values. Parsing rejects undeclared, duplicate, nested/traversal, malformed, mismatched, or over-limit content before restore. Library, annotations, settings, audiobook metadata, and narration preferences are separate bounded JSON sections. DataStore remains a separate persistence boundary: an atomic app-private journal captures prior settings, then Room applies its mutations and restore-operation marker in one transaction; startup recovery uses that marker to keep committed settings or roll back an interrupted restore. Imported books/audio, covers, neural models, generated audio, private paths, and user-visible checksum lists remain outside the archive. Existing unified v1 and separate library/annotation JSON imports remain supported.
